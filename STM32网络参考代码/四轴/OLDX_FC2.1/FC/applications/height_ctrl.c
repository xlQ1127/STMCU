...
#define LAND_TRY_PERCISE 50
float simgol_exp[3][LAND_TRY_PERCISE];
float simgol(float emax,float vmax,float amax,float Ps,float Pe,u8 sel)
{
// vmax=3;
//emax=0.01;
//amax=9.8*1;
...
int flag=1;
if(Dis<K){
    t1=t0;t2=2*t0;flag=0;}
else{
    t1=t0+ts;
    t2=2*t0+ts;
}
int t=0,i=0;
int per_step=t2/LAND_TRY_PERCISE*100;
for (t=0;t<t2*100;t=t+per_step){
	if(flag)    
		if(t<t0*100)
		simgol_exp[sel][i]=P_s+zf*K/(1+exp(-a*((float)t/100.-t0)));
		else if(t>t1*100)
		simgol_exp[sel][i]=P_e+zf*K*(1/(1+exp(-a*((float)t/100.-t1)))-1);    
		else        
		simgol_exp[sel][i]=P_s+zf*K/2*(1+a/2*((float)t/100.-t0));    		
	else
		if(t<t0*100)
		simgol_exp[sel][i]=P_s+zf*K/(1+exp(-a*((float)t/100.-t0)));
		else
...
static float t_land,t_now,R_spd;
//	switch(state)
//	{
//	  case 0:
//			 if(cnt++>4.0/T)
   ...
   switch(state)
	{
	  case 0:
			 out=2000;
			 if(cnt++>10.0/T)
			 {cnt2=cnt1=cnt=0;state=1;}
		break;
	  case 1:
			 if(cnt++>8.0/T)
			 {t_now=T;cnt2=cnt1=cnt=0;state=2; t_land=LIMIT(simgol(0.0001,1.69,9.8,(float)out/1000.,0.2,Zr),0.05,8);}
			 if(cnt1++>1/T){cnt1=0;
			 R_spd=LIMIT(RNG_Get_RandomRange(-6,6)/10.,0,0.6);}
				 out+=(-0.2+R_spd)*1000*T;
			 if(out<1200)
				 out=1200;
	  break;	
	  case 2:
       if(cnt++>t_land/T)
			 {cnt2=cnt1=cnt=0;state=3;}		
 			  t_now+=T;
			  cnt2=LIMIT(t_now/(t_land/LAND_TRY_PERCISE),0,LAND_TRY_PERCISE-1);
				out=LIMIT(simgol_exp[Zr][cnt2]*1000,300,1200);
    break;
    case 3:
        if(cnt++>8/T)
			 {cnt2=cnt1=cnt=0;state=0;}		
    break;		
	}
