...
/*------------------------------------pid控制------------------------------------*/
#define PIDMIX(X,Y,Z) Motor_Thr + pid_pitch*X + pid_roll*Y + pid_yaw*Z		 //	此处STM32F407上传感器与机体的横滚与俯仰是反的
...
/*---------------------------------------十字型-------------------------------------------*/
/*	MOTOR1=MOTORLimit(PIDMIX( 0,+1,-1));	 	//REAR	 后尾电机		
    MOTOR2=MOTORLimit(PIDMIX(-1, 0,+1));		//RIGHT 右边电机		

    MOTOR3=MOTORLimit(PIDMIX(+1, 0,+1));		//LEFT	 左边电机		
    MOTOR4=MOTORLimit(PIDMIX( 0,-1,-1));		//FRONT 前面电机						  */
/*---------------------------------------X型----------------------------------------------*/
/*	MOTOR1=MOTORLimit(PIDMIX(-1,+1,-1));	 	//REAR_R  后右电机
    ...
    MOTOR2=MOTORLimit(PIDMIX(-1,-1,-1));		//FRONT_R 前右电机
    MOTOR3=MOTORLimit(PIDMIX(-1,+1,+1));		//REAR_L  后左电机
    MOTOR4=MOTORLimit(PIDMIX(+1,-1,+1));		//FRONT_L 前左电机	  此处STM32F407上传感器与机体的横滚与俯仰是反的
												
/*	MOTOR1=MOTORLimit(PIDMIX(+1,+1,0));	 	//REAR_R  后右电机
    MOTOR2=MOTORLimit(PIDMIX(-1,-1,0));		//FRONT_R 前右电机
    MOTOR3=MOTORLimit(PIDMIX(-1,+1,0));		//REAR_L  后左电机
    MOTOR4=MOTORLimit(PIDMIX(+1,-1,0));		//FRONT_L 前左电机	  此处STM32F407上传感器与机体的横滚与俯仰是反的
			  							 		  */
}
...
{
	float servo_kp=100/9;

	if(roll_data>45)
	{
		roll_data=45;
	}
	else if(roll_data<-45)
	{
		roll_data=-45;
	}
	...
	else if(pitch_data<-45)
	{
		pitch_data=-45;
	}

	servo_Roll=ServoRoll_Middle+servo_kp*(platform_r_des-roll_data);		  //期望减去测量   云台舵机补偿输出
	servo_Pitch=ServoPitch_Middle+servo_kp*(platform_p_des-pitch_data);
