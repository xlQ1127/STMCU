...
#define SCL_PIN		A15	//液晶时钟引脚定义		
#define SDA_PIN	    A16 //液晶数据引脚定义
#define RES_PIN		B16	//液晶复位引脚件定义		
#define DC_PIN	    B17 //液晶命令引脚定义
#define CS_PIN	    A14 //液晶片选引脚定义

#define TFT_SCL(x)   	gpio_set(SCL_PIN,x);
#define TFT_SDA(x) 	    gpio_set(SDA_PIN,x);
#define TFT_RES(x)   	gpio_set(RES_PIN,x);
#define TFT_DC(x)   	gpio_set(DC_PIN ,x);
#define TFT_CS(x) 	    gpio_set(CS_PIN ,x);
