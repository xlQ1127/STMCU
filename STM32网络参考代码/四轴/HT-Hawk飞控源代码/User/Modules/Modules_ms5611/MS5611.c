...
void Sethigh_Mode(void) 
{
  ...
  Acc_speed_Filterbuffer[p] = (sensor.acc.averag.z * COS(angle.pitch/RtA) * COS(angle.roll/RtA) - sensor.acc.averag.x * SIN(angle.pitch/RtA) + sensor.acc.averag.y * SIN(angle.roll/RtA) - sensor.acc.quiet.z)*Acc_G; 
	p++;
	if(p== SAMPLE_NUM) p = 0;
	for(int k=0;k< SAMPLE_NUM;k++)
	{
	   sum +=Acc_speed_Filterbuffer[k];
	}
  SpeedZ += sum/SAMPLE_NUM; //加速度积分=速度
	
	SpeedZ = SpeedZ*0.98;
	

	RC_Data.THROTTLE -=  (SpeedZ +(SpeedZ - SpeedZ_old));
	
	SpeedZ_old = SpeedZ;
	
	if(RC_Data.THROTTLE <=0)  RC_Data.THROTTLE = 0;
	else if(RC_Data.THROTTLE >= 3000) RC_Data.THROTTLE=3000;
}
