...
{
     HAL_UART_Transmit(&hWIFI, &value, 1, 0xFF); //串口发送函数
...
void all_data_update(void)
{
  ...
  if(VisitorNum >= 0)
  {
    ...
    mcu_dp_value_update(DPID_UNLOCK_ID,  VisitorNum); //VALUE型数据上报;
    VisitorNum = -1;
  }

  mcu_dp_enum_update(DPID_ALARM, 0); //枚举型数据上报;
  mcu_dp_value_update(DPID_UNLOCK_REQUEST, 3); //VALUE型数据上报;
  mcu_dp_enum_update(DPID_BATTERY, Battery.Status); //枚举型数据上报;
}
