...
typedef struct 
{
	uint32_t(*read_point)(uint16_t,uint16_t);				//uint32_t read_point(uint16_t x,uint16_t y)						读点函数
	void(*draw_point)(uint16_t,uint16_t,uint32_t);			//void draw_point(uint16_t x,uint16_t y,uint32_t color)		    画点函数
 	void(*fill)(uint16_t,uint16_t,uint16_t,uint16_t,uint32_t);		///void fill(uint16_t sx,uint16_t sy,uint16_t ex,uint16_t ey,uint32_t color) 单色填充函数 	 
