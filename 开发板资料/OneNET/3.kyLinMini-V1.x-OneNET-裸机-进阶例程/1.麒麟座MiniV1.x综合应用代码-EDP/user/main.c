...
void DATA_Task(void)
{
	...
	switch(onenet_info.sendData)
	{
		...
		case SEND_TYPE_HEART:
			...
			onenet_info.sendData = OneNet_SendData_Heart();		//心跳检测
		
		break;
		
		case SEND_TYPE_PICTURE:
		
			onenet_info.sendData = OneNet_SendData(FORMAT_TYPE2, NULL, NULL, NULL, 0);
...
void NET_Task(void)
{
	...
	if(onenet_info.netWork == 0)
	{
		if(!onenet_info.netWork && (check_info.NET_DEVICE_OK == DEV_OK))			//当没有网络 且 网络模块检测到时
		{
			...
			{
				OneNet_DevLink(onenet_info.devID, onenet_info.apiKey);			//接入平台
