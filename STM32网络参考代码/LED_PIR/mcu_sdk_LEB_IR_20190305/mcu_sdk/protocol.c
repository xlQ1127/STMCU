...
void all_data_update(void)
{
  ...
  mcu_dp_bool_update(DPID_LED1,当前LED1); //BOOL型数据上报;
...
/*****************************************************************************
函数名称 : dp_download_led1_handle
功能描述 : 针对DPID_LED1的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_led1_handle(const unsigned char value[], unsigned short length)
{
  //示例:当前DP类型为BOOL
  unsigned char ret;
  //0:关/1:开
  ...
  led1 = mcu_get_dp_download_bool(value,length);
  if(led1 == 0)
  {
    //开关关
  }
  ...
  //处理完DP数据后应有反馈
  ret = mcu_dp_bool_update(DPID_LED1,led1);
  if(ret == SUCCESS)
    return SUCCESS;
  else
    return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
  ...
  ***********************************/
  ...
  switch(dpid)
  {
    case DPID_LED1:
      //LED1处理函数
      ret = dp_download_led1_handle(value,length);
      break;
