...
const char *topics[] = {"kylinMiniTest", "pcTopic"};
...
void HEART_Task(void)
{
	...
	onenet_info.sendData = SEND_TYPE_HEART;		//心跳连接
...
void DATA_Task(void)
{
	...
	switch(onenet_info.sendData)
	{
		case SEND_TYPE_DATA:
			...
			onenet_info.sendData = OneNet_SendData(FORMAT_TYPE3, NULL, NULL, dataStream, dataStreamCnt);	//上传数据到平台
			
		break;
		
		case SEND_TYPE_SUBSCRIBE:
		
			onenet_info.sendData = OneNet_Subscribe(topics, 2);											//订阅主题
		
		break;
		
		case SEND_TYPE_UNSUBSCRIBE:
		
			onenet_info.sendData = OneNet_UnSubscribe(topics, 2);										//取消订阅的主题
		
		break;
		
		case SEND_TYPE_PUBLISH:
		
			onenet_info.sendData = OneNet_Publish("pcTopic1", "New SDK Publish Test");					//发布主题
...
void NET_Task(void)
{
	...
	if(onenet_info.netWork == 0)
	{
		if(!onenet_info.netWork && (check_info.NET_DEVICE_OK == DEV_OK))			//当没有网络 且 网络模块检测到时
		{
			...
			{
				OneNet_DevLink(onenet_info.devID, onenet_info.proID, onenet_info.auif);//接入平台
				
				if(onenet_info.netWork)
				{
					onenet_info.sendData = SEND_TYPE_SUBSCRIBE;					//接入成功则订阅主题
...
void NET_Timer(void)
{
	...
	if(onenet_info.netWork == 0)											//如果网络断开
	{
