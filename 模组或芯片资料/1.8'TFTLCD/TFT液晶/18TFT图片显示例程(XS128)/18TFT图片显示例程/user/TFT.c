...
#define scl(x)  SCL_PutVal(x)
#define sda(x)  SDA_PutVal(x)
#define reset(x)RES_PutVal(x)
#define dc(x)   DC_PutVal(x)
#define cs(x)   CS_PutVal(x)
...
void  LCD_WriteData_16Bit(unsigned int Data)
{
	...
	SPI_WriteData((byte)Data); 			//写入低8位数据
	cs(1);
...
{	
  Reset();//Reset before LCD Init.
	...
	Lcd_WriteIndex(0xB4); //Column invedcion 
	Lcd_WriteData(0x07); 
...
void lcd_showuint8(word x,word y,byte dat)
{
	byte a[3];
	byte i;
...
void lcd_showuint16(word x,word y,word dat)
{
	byte a[5];
	byte i;
