...
void USART1_Configuration(void) //串口配置初始化：USART1
{
...
void  USART1_SendData(unsigned char SendData) //防卡函数
{
while(USART_GetFlagStatus(USART1, USART_FLAG_TXE) == RESET);//检测上次发送是否完成；	
...
void Tim2_init()//PWM产生
{
 ...
 void TIM4_INT(void) //计算周期计时器
{    
  RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM4, ENABLE);
	TIM4->ARR=20000; //设定计数器自动重装值40ms 
	TIM4->PSC=63;   //预分频器64，得到500khz的计数时钟：定时器时钟/ (TIMx_PSC +1)  
	TIM4->CR1|=0x01;    //使能定时器
} 



void TIM3_INT(void)//2.5ms
...
{ 
  ...
  NVIC_InitStructure.NVIC_IRQChannelSubPriority = 2;        //响应优先级0-3 （排队）
...
void GPIO_Configuration(void) //各个模块GPIO初始化
{
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0|GPIO_Pin_1;//  PB0-1-LED
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz; 
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 
  GPIO_Init(GPIOB, &GPIO_InitStructure); 
  ...
  GPIO_Init(GPIOA, &GPIO_InitStructure);
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2;//  24L01 CSN CE
...
uint32_t GET_Time(uint32_t * lasttime)//返回当前systick计数器值,32位
{
