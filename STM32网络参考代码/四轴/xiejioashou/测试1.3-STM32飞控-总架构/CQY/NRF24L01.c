...
#define CE1 GPIO_SetBits(GPIOA, GPIO_Pin_4)
#define CE0 GPIO_ResetBits(GPIOA, GPIO_Pin_4)

#define CSN1 GPIO_SetBits(GPIOB, GPIO_Pin_11)
#define CSN0 GPIO_ResetBits(GPIOB, GPIO_Pin_11)


#define IRQ1 GPIO_SetBits(GPIOB, GPIO_Pin_10)
#define IRQ0 GPIO_ResetBits(GPIOB, GPIO_Pin_10)
...
{//发送模式
  ...
  NRFWriteTxDate(W_REGISTER+TX_ADDR,TxAddr,TX_ADDR_WITDH);//写寄存器指令+接收地址使能指令+接收地址+地址宽度
	...
	/******下面有关寄存器配置**************/
  	...
  	NRFWriteReg(W_REGISTER+SETUP_RETR,0x01);  //自动重发延时等待250us+86us，自动重发1次
