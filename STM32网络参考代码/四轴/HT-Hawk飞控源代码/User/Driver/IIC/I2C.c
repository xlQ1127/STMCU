...
* Input          : None  【PB10 11】
* Output         : None   MPU6050使用
* Return         : None
****************************************************************************** */
void I2C_INIT(void)
{
  GPIO_InitTypeDef  GPIO_InitStructure;          //======================【IIC引脚初始化】
 
  RCC_APB2PeriphClockCmd(RCC_GPIO_I2C, ENABLE);  //使能GPIOB时钟
  
  GPIO_InitStructure.GPIO_Pin =  SCL_PIN;        //======================【GPIOB-10 SCL】
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_OD;  
  GPIO_Init(GPIO_I2C, &GPIO_InitStructure);

  GPIO_InitStructure.GPIO_Pin =  SDA_PIN;        //======================【GPIOB-11 SDA】
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_OD;
  GPIO_Init(GPIO_I2C, &GPIO_InitStructure);
  delay(0XFFF);
...
//===============================【IIC复合模式】==================================
void I2C_Read(u8 addr_, u8 reg_, u8 len, u8 *buf)  //【设备地址  存储单元地址  数据长度  数据存放地址】
{
	u8 i;
	 // 起始信号
	I2C_Start();
