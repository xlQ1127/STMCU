...
{
	 HAL_SPI_DeInit(&hSDSPI);
	
  hSDSPI.Instance = SPI1;
  hSDSPI.Init.Mode = SPI_MODE_MASTER;
  hSDSPI.Init.Direction = SPI_DIRECTION_2LINES;
  hSDSPI.Init.DataSize = SPI_DATASIZE_8BIT;
  hSDSPI.Init.CLKPolarity = SPI_POLARITY_HIGH;
  hSDSPI.Init.CLKPhase = SPI_PHASE_2EDGE;
  hSDSPI.Init.NSS = SPI_NSS_SOFT;
  hSDSPI.Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_256;
  hSDSPI.Init.FirstBit = SPI_FIRSTBIT_MSB;
  hSDSPI.Init.TIMode = SPI_TIMODE_DISABLE;
  hSDSPI.Init.CRCCalculation = SPI_CRCCALCULATION_DISABLE;
  hSDSPI.Init.CRCPolynomial = 10;
  if (HAL_SPI_Init(&hSDSPI) != HAL_OK)
  {
    Error_Handler();
  }}
//SD卡正常工作的时候,可以高速了
void SD_SPI_SpeedHigh(void)
{
  ...
  hSDSPI.Init.CLKPhase = SPI_PHASE_2EDGE;
  hSDSPI.Init.NSS = SPI_NSS_SOFT;
  hSDSPI.Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_4;
  hSDSPI.Init.FirstBit = SPI_FIRSTBIT_MSB;
  hSDSPI.Init.TIMode = SPI_TIMODE_DISABLE;
  hSDSPI.Init.CRCCalculation = SPI_CRCCALCULATION_DISABLE;
  hSDSPI.Init.CRCPolynomial = 10;
...
{
	uint8_t Rx = 0;
	HAL_SPI_TransmitReceive(&hSDSPI, &data, &Rx, 1, 0xFF);
	return Rx;
}
