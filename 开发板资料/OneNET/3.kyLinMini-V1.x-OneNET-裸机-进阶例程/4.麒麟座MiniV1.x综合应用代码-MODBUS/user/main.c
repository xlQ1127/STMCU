...
void DATA_Task(void)
{
	...
	switch(onenet_info.sendData)
	{
		case SEND_TYPE_DATA:
			...
			{
				onenet_info.sendData = OneNet_SendData(OneNet_GetListHeadBuf(), OneNet_GetListHeadLen());	//上传数据到平台
...
void NET_Task(void)
{
	...
	if(onenet_info.netWork == 0)
	{
		if(!onenet_info.netWork && (check_info.NET_DEVICE_OK == DEV_OK))			//当没有网络 且 网络模块检测到时
		{
			...
			{
				OneNet_DevLink(onenet_info.serial, onenet_info.pswd, onenet_info.proID);//接入平台
