...
void I2C_MPU6050_WriteData(uint16_t Addr, uint8_t Reg, uint8_t Value)
{
  ...
  status = HAL_I2C_Mem_Write(&hi2c1, Addr, (uint16_t)Reg, I2C_MEMADD_SIZE_8BIT, &Value, 1, 0xFFFF);
...
HAL_StatusTypeDef I2C_MPU6050_ReadBuffer(uint16_t Addr, uint8_t Reg, uint16_t RegSize, uint8_t *pBuffer, uint16_t Length)
{
  ...
  status = HAL_I2C_Mem_Read(&hi2c1, Addr, (uint16_t)Reg, RegSize, pBuffer, Length, 0xFF);
...
HAL_StatusTypeDef I2C_MPU6050_WriteDatas(uint16_t Addr, uint8_t Reg,uint8_t len, uint8_t *Value)
{
  ...
  status = HAL_I2C_Mem_Write(&hi2c1, Addr, (uint16_t)Reg, I2C_MEMADD_SIZE_8BIT, Value, len, 0xFFFF);
...
{ 
  return (HAL_I2C_IsDeviceReady(&hi2c1, DevAddress, Trials, 0xFFFF));
}
