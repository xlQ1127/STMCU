...
{  
 uint8_t *pData;  
    pData = &I2C_Command;  
    if(HAL_I2C_Mem_Write(&hi2c1,0x78,0x00,I2C_MEMADD_SIZE_8BIT,pData,1,0xFFFF)!= HAL_OK )
    {
				 I2Cx_ErrHandler();
				}
}  
...
{  
 uint8_t *pData;  
    pData = &I2C_Data;  
     if(HAL_I2C_Mem_Write(&hi2c1,0x78,0x40,I2C_MEMADD_SIZE_8BIT,pData,1,0xFFFF)!= HAL_OK )
     { 
					 I2Cx_ErrHandler();
					}
}  
...
void OLED_PutPixel(uint8_t x, uint8_t y, uint8_t Color)
{
	uint8_t PageAddr;
	uint8_t ColAddr;
	...
	ColAddr  = y;
		...
		OLED_WR_Byte ( 0xB0 + PageAddr                , OLED_CMD );				/* ÉèÖÃÒ³µØÖ·£¨0~7£© */
		OLED_WR_Byte ( 0x00 + (ColAddr & 0x0F)        , OLED_CMD );			/* ÉèÖÃÁÐµØÖ·µÄµÍµØÖ· */
		OLED_WR_Byte ( 0x10 + ((ColAddr >> 4) & 0x0F) ,OLED_CMD );	/* ÉèÖÃÁÐµØÖ·µÄ¸ßµØÖ· */
