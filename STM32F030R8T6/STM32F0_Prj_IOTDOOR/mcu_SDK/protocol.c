...
{
  extern void uart2_sendchar(unsigned char ch);
  uart2_sendchar(value);	                                //串口发送函数
...
void all_data_update(void)
{
  ...
  {
    mcu_dp_enum_update(DPID_UNLOCK_TYPE, 1); //枚举型数据上报;
    mcu_dp_value_update(DPID_UNLOCK_ID, SystemData.VisitorNum); //VALUE型数据上报;
    SystemData.VisitorNum = -1;
  }

  mcu_dp_enum_update(DPID_ALARM, 0); //枚举型数据上报;
  mcu_dp_value_update(DPID_UNLOCK_REQUEST, 3); //VALUE型数据上报;
  mcu_dp_enum_update(DPID_BATTERY, SystemData.BatteryStatus); //枚举型数据上报;
