...
void NRF24L01_Init(uint8_t Channal, uint8_t Mode)
{
  ...
  NRF24L01_Write_Reg(NRF_WRITE_REG + EN_RXADDR, 0x01);  //使能通道0的接收地址
	 ...
	 NRF24L01_Write_Reg(NRF_WRITE_REG+SETUP_RETR,0xff);//设置自动重发间隔时间:4000us + 86us;最大自动重发次数:15次  0x1a 设置自动重发间隔时间:500us;最大自动重发次数:10次
...
uint8_t NRF24L01_TxPacket(uint8_t *txbuf)
{
  ...
  NRF24L01_Write_Buf(WR_TX_PLOAD,txbuf,TX_PLOAD_WIDTH);//写数据到TX BUF  32个字节
	...
	while(NRF24L01_IRQ_PIN_READ()!= GPIO_PIN_RESET );//等待发送完成
