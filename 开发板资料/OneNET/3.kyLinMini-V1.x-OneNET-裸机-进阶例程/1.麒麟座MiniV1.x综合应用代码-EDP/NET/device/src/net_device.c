...
NET_DEVICE_INFO net_device_info = {0, 0, NULL, NULL, NULL, NULL, NULL, 0, 0, 0, 0, 0, 1, 0};
...
void NET_DEVICE_IO_Init(void)
{
	...
	gpio_initstruct.GPIO_Pin = GPIO_Pin_5;					//GPIOB5-复位
...
unsigned int NET_DEVICE_GetTime(void)
{
	...
	if(NET_DEVICE_SendCmd("AT+CIPSTART=\"UDP\",\"edu.ntp.org.cn\",123\r\n", "CONNECT") == 0)
	{
		...
		{
			dataPtr = net_device_info.cmd_ipd;
...
_Bool NET_DEVICE_GetLinkIP(unsigned char protocol, char *ip, char *port)
{
	...
	if(NET_DEVICE_SendCmd("AT+CIPSTART=\"TCP\",\"183.230.40.33\",80\r\n", "CONNECT") == 0)
	{
		...
		while(--timeOut)
		{
			dataPtr = strstr(net_device_info.cmd_ipd, "no-cache");				//找到最后的关键词
...
_Bool NET_DEVICE_Init(unsigned char protocol, char *ip, char *port)
{
	...
	switch(net_device_info.initStep)
	{
		case 0:
...
_Bool NET_DEVICE_SendData(unsigned char *data, unsigned short len)
{
	...
	while(!net_device_info.send_ok && --time_out)		//等待上一次的数据成功发送
		DelayXms(10);
...
unsigned char *NET_DEVICE_GetIPD(unsigned char *dataPtr)
{
	...
	if(dataPtr_tp != NULL)
	{
		...
		else
		{
			...
			net_device_info.data_bytes = atoi(bytes);					//转换
...
unsigned char NET_DEVICE_Check(void)
{
	...
	{
		if(strstr(net_device_info.cmd_resp, "STATUS:2"))				//获得IP
		{
			UsartPrintf(USART_DEBUG, "ESP8266 Got IP\r\n");
			return 2;
		}
		else if(strstr(net_device_info.cmd_resp, "STATUS:3"))			//建立连接
		{
			UsartPrintf(USART_DEBUG, "ESP8266 Connect OK\r\n");
			return 0;
		}
		else if(strstr(net_device_info.cmd_resp, "STATUS:4"))			//失去连接
		{
			UsartPrintf(USART_DEBUG, "ESP8266 Lost Connect\r\n");
			return 1;
		}
		else if(strstr(net_device_info.cmd_resp, "STATUS:5"))			//物理掉线
		{
